    ...
    @FXML 
     ...
     public void onclickCommander(ActionEvent event){
       //   
          ...
          ObservableList<LigneCommande> selection = wishlist.getSelectionModel().getSelectedItems() ; 
        for(LigneCommande l : selection){
            Commande c = new Commande(1, l.getId_ligne(), l.getQuantite()*l.getPrix_produit()) ;
            ParentService ps = new ParentService() ; 
            Parent p = ps.search(1) ; 
     ...
     prs.update(pr2) ; 
     Parent p2 = new Parent(p.getId_user(), p.getCin(), p.getNom(), p.getPrenom(), p.getPseudo(), p.getMdp(), p.getEmail(), p.getAdresse(), p.getMontant()-c.getTotal(), p.getType(), p.getImage()); 
     ps.update(p2) ;  } 
     //else livraison.fxml 
